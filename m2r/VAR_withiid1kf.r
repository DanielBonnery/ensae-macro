VAR_withiid <- function(data, prior, gssettings, printout){
...
[Tw,Nw] <- size(data.w); if (Tw!=T, error('y and w have different lengths'), else clear Tw, }){
Nm <- prior.Nm
Ny <- N - Nm
P <- prior.lags
...
if (!isfield(gssettings,'saveevery'), gssettings.saveevery <- 1; }){
if (!isfield(gssettings,'waitbar'), gssettings.waitbar <- 1; }){
if (!isfield(gssettings,'computemarglik'), gssettings.computemarglik <- 0; }){
...
if (isfield(prior,'minnesota') && isfield(prior.minnesota,'tightness')){
    ...
    if (!isinf(prior.minnesota.tightness)){
        ...
        } else if (length(prior.minnesota.mvector) > N){
            warning('Minnesota prior: mvector too long, truncating')##ok<WNTAG>
        } else if (length(prior.minnesota.mvector)==Ny){
    ...
    if (isfield(prior,'simsdummy')){
        if (!isfield(prior.simsdummy,'oneunitroot') prior.simsdummy.oneunitroot <- 0; }){
        if (!isfield(prior.simsdummy,'oneunitrootc') prior.simsdummy.oneunitrootc <- 0; }){
        if (!isfield(prior.simsdummy,'oneunitrooty') prior.simsdummy.oneunitrooty <- 0; }){
        if (!isfield(prior.simsdummy,'nocointegration') prior.simsdummy.nocointegration <- 0; }){
        if (prior.simsdummy.oneunitroot || prior.simsdummy.oneunitrootc || prior.simsdummy.oneunitrooty || any(prior.simsdummy.nocointegration)){
            ybar <- [zeros(1,Nm) mean(data.y(1:P,Nm+1:length(mean)),1)]; Xprior <- []; Yprior <- []
            ...
            prior.simsdummy.QinvB_reshaped <- Xprior'*Yprior(:,Nm+1:length(Yprior))*tempSigmayinv
    ...
    if (printout){
        ...
        if (isfield(prior,'simsdummy'), disp(prior.simsdummy); }){
        disp(prior)
    }
}
# store the prior
...
X <- [X data.w(P+1:},:)]# add the exogenous variables
result.Y <- Y
result.X <- X
...
if (gssettings.ndraws){
    ...
    # last preparations
    it_save <- 0
    if (isfield(gssettings,'waitbar') && gssettings.waitbar, waitbar_handle <- waitbar(0,'Gibbs sampler','Name','Gibbs sampler running'); }){
    timing_start <- now
    disp(['start: ',datestr(timing_start,0), '; total iterations: ', num2str(nalldraws)])

    # Gibbs sampler
    for (draw in 1:nalldraws){
        ...
        if (draw>gssettings.burnin && !rem(draw,gssettings.saveevery)){
            ...
            if (gssettings.computemarglik){
                ...
                temp <- BB(:,Nm+1:length(BB)) - prior.B
    ...
    if (gssettings.waitbar, close(waitbar_handle), }){
