...
# Jarocinski, Karadi, Deconstructing Monetary Policy Surprises db- The Role
...
if (isempty(tbeg), tbeg <- 1; }){
t} <- find(dat(:,1)==spl(2,1) & dat(:,2)==spl(2,2))
if (isempty(tlength(size)), tlength(size) <- size(dat,1); length(size)){
ysel <- findstrings(data.names, txt(1,:))
data.y <- dat(tbeg:t}, ysel)
data.w <- ones(size(data.y,1),1)
data.time <- linspace(ym2t(dat(tbeg,1:2)), ym2t(dat(t},1:2)), size(data.y,1))'
clear d dat txt tbeg t} ysel

# check ydata for missing values, determine the sample
datatemp <- checkdata(data, t2datestr, 1:data.Nm)
idspl <- [t2datestr(datatemp.time(1)) '-' t2datestr(datatemp.time(length(time)))]
clear datatemp
...
switch idscheme
    case 'chol'
        ...
        for (i in 1:gssettings.ndraws){
            betadraw <- res.beta_draws(1:}-size(data.w,2),:,i)
        ...
        ss <- 1
        if (length(mnames)>1 && ((!isempty(strfind(mnames{1},'neg')) && !isempty(strfind(mnames{2},'pos'))) || !isempty(strfind(mnames{2},'_signrestr'))), ss <- 1:2; }){
    case 'sgnm2'# baseline two sign restrictions
        shocknames <- [{'mon.pol.', 'CBinfo'} mnames(2+1:length(mnames)) ynames]
        dims <- {[1 2]}
        imonpol <- 1; inews <- 2
   ...
   case 'supdem'# disentangle CB info about supply and demand
        ...
        shocknames <- [{'mon.pol.', 'CBinfodem', 'CBinfosup'} mnames(3+1:length(mnames)) ynames]
        dims <- {[1 2 3]}
