checkdata <- function(data, t2datestr, ignorecols){
...
N <- length(data.names)
[T,N2] <- size(data.y)
if (N2!=N, error('data.names inconsistent with data.y'), }){
...
for (n in 1:N){
       ...
       if (isempty(ilast), ilast <- T; }){
       nmid <- sum(isnan(data.y(ifirst:ilast,n)))){
       ibeg}(n,:) <- [ifirst ilast nmid]){
       #fprintf(1,'#-18s #s-#s   #internal NaNs: #d\n', data.names{n}, t2datestr(data.time(ifirst)), t2datestr(data.time(ilast)), nmid);
}
disp([char(data.names) repmat(' ',N,1) t2datestr(data.time(ibeg}(:,1))) repmat(' - ',N,1) t2datestr(data.time(ibeg}(:,2))) repmat('   #internal NaNs: ',N,1) num2str(ibeg}(:,3))])
...
if (ifirst>1 || ilast<size(data.y,1)){
    disp(' ')
    disp(['Truncating sample from: ' t2datestr(data.time(1)) '-' t2datestr(data.time(length(time)))])
...
if (isfield(data,'w'), dataout.w <- data.w(ifirst:ilast,:); }){
