plot_irfs_draws_robust <- function(){
...
if (isempty(shocknames), shocknames <- ynames; }){
...
if (whichplots(3)){
    ...
    for (iv in 1:Nv){
        ...
        for (is in 1:Ns){
            ...
            if (is==1, ylabel(strrep(ynames{v},'_',' ')), }){
    ...
    for (iv in 1:Nv){
        ...
        for (is in 1:Ns){
            ...
            if (squeeze(irfs_l_draws(varstoplot(iv),shockstoplot(is),1:2,1))==[0;0], ylims(is,:) <- 0; }){
        }
...
make_robust_irfbands <- function(v,s,irfs_l_draws,irfs_u_draws,credibility){
# PURPOSE: construct
