resirfssign <- function(res, nstep, dims_array, test_restr, b_normalize, max_attempts){
...
for (i in 1:ndraws){
    ...
    if (isnan(irfs_draws(1,1,1,i)); failures <- failures+1; }){
...
mmult3d2d <- function(mat3d,mat2d){
...
[a b c] <- size(mat3d)
[b1 d] <- size(mat2d)
if (b1! in b error('matrices not conformable!'); }){){
