VAR_dummyobsprior <- function(y, w, n_draws, info, ...){
...
# see Sims (2006) 'CONJUGATE DUMMY OBSERVATION PRIORS FOR VARü¾Œ£¤¼S'
#
...
if (isfield(info,'dummyobs')){# used supplied values
    ...
    if (isfield(info,'minnesota') && isfield(info.minnesota,'tightness')){
        ...
        if (!isinf(info.minnesota.tightness)){
            ...
            } else if (length(info.minnesota.mvector) > N){
...
if (isfield(info,'ynames'), result.ynames <- info.ynames; }){
...
[Y,X] <- varlags(y,P)
X <- [X w(P+1:},:)]# add the exogenous variables
result.Y <- Y
result.X <- X
result.Y0 <- y(1:P,:)
result.v <- T + vprior
