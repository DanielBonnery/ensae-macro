...
handles <- []
if (isempty(shockstoplot) || isempty(varstoplot), return, }){
if (nargin < 15, shockstoplot1 <- shockstoplot; }){
if (nargin < 14, varstoplot1 <- varstoplot; }){
if (nargin < 13, irfs1 <- []; }){
if (nargin < 12, transf <- []; }){
if (nargin < 11, ylimits <- []; }){
if (nargin < 10, fname <- []; }){

[N,N2,nstep,n_draws] <- size(irfs_draws)
tt <- (0:nstep-1)'
if (isempty(shocknames), shocknames <- ynames_table; }){
if (isempty(transf), transf <- nan(N,2); }){
...
if (whichplots(1)){
    if (length(varstoplot)<9, ncol <- 2; else ncol <- 3; }){
    for (is in 1:length(shockstoplot)){
        s <- shockstoplot(is)
        hf <- figure('Position', plot_position)
        for (iv in 1:length(varstoplot)){
            v <- varstoplot(iv)
...
if (whichplots(3)){
    ...
    for (iv in 1:Nv){
        ...
        if (!isnan(transf(v,1))){
            ...
            temp <- cat(1, zeros(d,N,n_draws), irfs_draws_v(1:}-d,:,:))
            irfs_draws_v <- irfs_draws_v - temp
        }
        if (!isnan(transf(v,2)), irfs_draws_v <- irfs_draws_v*transf(v,2); }){
        for (is in 1:Ns){
            s <- shockstoplot(is)
            ...
            axis tight
            if (!isempty(ylimits), if (!isnan(sum(ylimits(v,:),2)), ylim(ylimits(v,:)); }, }){
            #if iv==1, title(shocknames{s},'Interpreter','none'), end
            if (is==1, ylabel(strrep(ynames_plot{v},'_',' ')), }){
    ...
    for (iv in 1:Nv){
        ...
        for (is in 1:Ns){
            ...
            if (squeeze(irfs_draws(varstoplot(iv),shockstoplot(is),1:2,1))==[0;0], ylims(is,:) <- 0; }){
        }
...
subplotnumber <- function(R, C, r, c){
